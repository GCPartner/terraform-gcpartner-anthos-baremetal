...
resource "google_compute_network" "gcp_network" {
  ...
  project                 = local.project_id
  name                    = "gcp-network-${random_string.project_suffix.result}"
...
resource "google_compute_address" "bastion_ip" {
  ...
  name    = "bastion-ip-${random_string.project_suffix.result}"
  project = local.project_id
...
resource "google_compute_disk" "default" {
  ...
  project = local.project_id
  name    = format("disk-${random_string.project_suffix.result}-%02d", count.index + 1)
  type    = "pd-ssd"
  zone    = var.gcp_zone
